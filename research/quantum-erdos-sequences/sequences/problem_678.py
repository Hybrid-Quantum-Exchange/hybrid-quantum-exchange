"""
Erdos problem #678 (from manman4/erdosproblems data/problems.yaml).

Metadata found for problem 678:
    prize: no
    informal_status: proved (Lean-formalized 2026-01-07)
    oeis: ["possible"]
    tags: ["number theory"]

LIMITATION (read before trusting the PASS below): the "oeis" field for
problem 678 is the literal placeholder string "possible", not a real OEIS
...
finite instance derivable from #678 itself is available in the cloned
data. This script is an honest best-effort fallback, not a circuit that
tests anything specific to Erdos problem #678's actual mathematical
content.

Chosen fallback property (classical, finite, computable, and a fair task
for Grover's algorithm): "which 4-bit integers in [0, 15] are prime?" is
used as a small, well-defined search problem with a verifiable classical
answer, exercising the same primitive (amplitude amplification of marked
basis states) that a real Erdos-678-derived oracle would need if a
concrete OEIS sequence had been available. It is explicitly NOT derived
from problem 678's mathematics.

Classical answer (computed here, not copied): primes in range(16) are
{2, 3, 5, 7, 11, 13} -> marked bitstrings (4-bit, little-endian in circuit
convention) for those 6 values out of 16.

The script:
  1. Computes the classical set of primes in [0, 15] via trial division.
  2. Builds a Grover search circuit (4 qubits) whose oracle phase-flips
     exactly the prime-valued basis states, with the standard diffuser,
     run for the optimal number of Grover iterations.
  ...
  4. Prints PASS if the highest-probability measured states are exactly
     the classical prime set (up to the expected Grover amplification),
     else FAIL.
...
def classical_primes(n_max):
    """Trial-division primality test; return sorted list of primes < n_max."""
    primes = []
...
N_QUBITS = 4
N = 2 ** N_QUBITS  # 16
MARKED = classical_primes(N)  # classical ground truth: [2,3,5,7,11,13]
