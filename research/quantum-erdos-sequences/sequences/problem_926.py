"""
Erdos problem #926 -- quantum-testable sequence attempt (honest best-effort, with a
documented limitation).

Source metadata (from erdosproblems/data/problems.yaml, entry "number: 926"):
    prize: "no"
...
The yaml's oeis field for problem 926 is the literal placeholder string
...
a real Grover search circuit on the ideal AerSimulator, searching for perfect
matchings among labeled graphs on 4 vertices. This is a generic graph-theory
demonstration in the spirit of the problem's tag, NOT a verification of
problem 926's actual mathematical content, and NOT tied to any specific OEIS
sequence. ran_ok/verified_against_classical below describe only this
substitute construction, honestly.
...
Classical property being tested:
    ...
    Marked set M = { graphs that are a perfect matching on {0,1,2,3}, i.e.
    every vertex has degree exactly 1 }. On 4 vertices there are exactly 3
    such graphs: {(0,1),(2,3)}, {(0,2),(1,3)}, {(0,3),(1,2)}. This is computed
    directly here in Python from first principles (brute-force enumeration of
    all 64 graphs and each vertex's degree), independent of any quantum
    computation.
...
def is_perfect_matching(bits):
    """bits: length-6 tuple of 0/1, bits[i] = presence of EDGES[i].
    True iff every vertex has degree exactly 1."""
...
for n in range(N_STATES):
    if is_perfect_matching(int_to_bits(n)):
        marked.append(n)

M = len(marked)
assert M > 0 and M < N_STATES, "sanity check: nontrivial marked set"
print(f"Classical enumeration: {M} of {N_STATES} labeled graphs on K4 are perfect matchings.")
...
print(f"Fraction of shots landing on a perfect matching: {success_shots / total_shots:.3f}")

quantum_found_marked = best_int in marked

if quantum_found_marked:
    print("PASS: Grover search's top outcome is a classically verified perfect matching.")
else:
    print("FAIL: Grover search's top outcome is NOT a perfect matching.")
